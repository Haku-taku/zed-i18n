...
- **Which-key surfaces**: Use `which-key 選單` for both `which-key popup` and `which-key menu` when they refer to the same feature, including legacy settings documentation. Keep `按鍵繫結預覽彈出視窗` for the separate binding preview popover.
