...
macOS와 Linux에서는 Homebrew cask로 설치할 수 있습니다.
...
Linux의 Homebrew 설치본은 Zed 자체 자동 업데이트를 사용할 수 없으며, `brew upgrade --cask zed-i18n`으로 업데이트할 수 있습니다. 앱 메뉴 항목이 자동으로 등록되지 않으므로 `zed-i18n`으로 실행해주세요.
...
Linux와 Windows에서 애플리케이션 메뉴(`app_menu::OpenApplicationMenu`)의 단축키를 직접 설정할 때 `File`, `Edit`, `View` 같은 고정된 영문 메뉴 이름을 사용하면 UI 언어를 변경해도 단축키가 계속 동작합니다. 설정 예시는 [메뉴 단축키 설정 가이드(영문)](../menu-access-keys.md)를 참고하세요.
