...
Linux と Windows でアプリケーションメニュー (`app_menu::OpenApplicationMenu`) のショートカットを独自に設定する場合は、`File`、`Edit`、`View` などの固定された英語のメニュー名を使用できます。これにより、UI の言語を変更してもショートカットが引き続き機能します。設定例については、[アプリケーションメニューのショートカット設定ガイド（英語）](../menu-access-keys.md)を参照してください。
