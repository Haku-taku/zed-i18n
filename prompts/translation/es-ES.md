...
- Do not apply glossary entries as blind string replacements. Before using a glossary row, identify the source term's role in this string: command/action verb, object/concept noun, adjective/modifier, named UI surface, or display/layout mode. Use English word order plus `kind`, `call`, `occurrences`, and `code_context` to choose the matching glossary context, then adapt the chosen term naturally to Spanish grammar.
...
- **Which-key surfaces**: Use `menú which-key` for both `which-key popup` and `which-key menu` when they refer to the same feature, including legacy settings documentation. The pending-keystrokes binding preview popover is a separate surface and remains a preview popup, not the which-key menu.
- **Watcher diagnostics**: Keep the `vigilancia` family for filesystem watching and its events/notifications. Avoid person-like `vigilante`. Review the tab title, explanatory text, empty state, and action description together when changing this terminology.
...
- **Declaration / Implementation / Type Definition**: `Declaración` / `Implementación` / `Definición de tipo` for code navigation. (Reference and Definition are in the glossary.)
