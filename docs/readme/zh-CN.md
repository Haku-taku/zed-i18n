...
在 macOS 和 Linux 上，你可以通过 Homebrew cask 安装。
...
在 Linux 上，通过 Homebrew 安装的版本无法使用 Zed 自带的自动更新，请改用 `brew upgrade --cask zed-i18n` 进行更新。由于不会自动创建应用菜单项，请使用 `zed-i18n` 启动。
...
在 Linux 和 Windows 上手动设置应用菜单（`app_menu::OpenApplicationMenu`）快捷键时，可以使用 `File`、`Edit`、`View` 等固定的英文菜单名称。这样即使更改 UI 语言，快捷键也能继续生效。配置示例请参阅[应用菜单快捷键设置指南（英文）](../menu-access-keys.md)。
