...
在 macOS 和 Linux 上，你可以透過 Homebrew cask 安裝。
...
在 Linux 上，透過 Homebrew 安裝的版本無法使用 Zed 內建的自動更新，請改用 `brew upgrade --cask zed-i18n` 進行更新。由於不會自動建立應用程式選單項目，請使用 `zed-i18n` 啟動。
...
在 Linux 和 Windows 上手動設定應用程式選單（`app_menu::OpenApplicationMenu`）快速鍵時，可以使用 `File`、`Edit`、`View` 等固定的英文選單名稱。如此一來，即使變更 UI 語言，快速鍵仍可繼續使用。設定範例請參閱[應用程式選單快速鍵設定指南（英文）](../menu-access-keys.md)。
